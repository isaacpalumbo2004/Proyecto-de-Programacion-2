...
def generar_factura(venta, archivo_factura):
    """Genera una factura de la venta en un archivo de texto."""
    with open(archivo_factura, 'w', encoding='utf-8') as f:
        ...
        f.write(f"\n")
        f.write(f"{'Producto':<30} {'Cantidad':<10} {'Precio Unitario':<15} {'Total':<15}\n")
        f.write("-" * 70 + "\n")
        ...
        for producto, datos in venta['productos'].items():
            ...
            f.write(f"{producto:<30} {datos['cantidad']:<10} {datos['precio_unitario']:<15.2f} {total_producto:<15.2f}\n")
        
        f.write("-" * 70 + "\n")
        f.write(f"{'Total de la Venta':<55} {total_venta:<15.2f}\n")
...
def procesar_ventas_y_agrupar_por_cliente(archivo_ventas_bin, carpeta_facturas):
    ...
    for cliente, datos_venta in ventas_por_cliente.items():
        ...
        archivo_factura = f"facturas/factura_{cliente}_{fecha_limpia}.txt"
